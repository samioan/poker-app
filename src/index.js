...
// #region You don't actually need variables on the global scope of the file to do the evaluation
//VARIABLES

// #region export this to two different files under the folder name types and cardColor.js and then import them here
//Basic card stats
...
// #endregion export this to two different files under the folder types

//Player
const myCardHand = ["H14", "H12", "H10", "H13", "H11"]; //The player's card hand // KEEP only this line for function test input
...
// #endregion You don't actually need variables on the global scope of the file to do the evaluation

//FUNCTIONS
//Creating a card combining the card stats. Randomized for now for testing.
function cardMaker(cColor, cNumber) {
  return (
    cColor[Math.floor(Math.random() * cColor.length)] +
    cNumber[Math.floor(Math.random() * cNumber.length)]
  );
}

// Based on the lesson we've studied we better not use void functions.
// That means this function returns a new array of cards
// eg const hand = getFiveCards(); // You don't even need an input here
//Function that adds 5 cards to a hand for testing
function handAdder(aHand) {
  let i;

  for (i = 0; i < 5; i++) {
    aHand.push(cardMaker(cardColor, cardNumber)); // ypu can use forEach method
  }
}

// instead of mutating use pure principle here, get something and return something anew. Avoid mutation
...
function handsChecker(aColor, aHandStrength, aCurrentHand) {
  ...
  const j =
    String(aHandStrength[0]) +
    String(aHandStrength[1]) +
    String(aHandStrength[2]) +
    String(aHandStrength[3]) +
    String(aHandStrength[4]);
  if (k === "Y" && j === "1011121314") {
...
// we don't care for the CPU so far in this scope, we care for the correct evaluation of a hand
console.log("------------------");
